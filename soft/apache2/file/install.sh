...
if ! grep -q -F '_http_' /etc/newsyslog.conf; then
    ...
    echo "/var/log/http/access_http.log   httpd.httpd  600  10   1024  *     Z /var/run/httpd.pid SIGUSR1" >> /etc/newsyslog.conf
    echo "/var/log/http/error_http.log    httpd.httpd  600  10   1024  *     Z /var/run/httpd.pid SIGUSR1" >> /etc/newsyslog.conf
    echo "" >> /etc/newsyslog.conf
    pkill -HUP syslogd
fi
