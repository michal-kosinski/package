...
if ! grep -q -F '_php4_' /etc/newsyslog.conf; then
    echo "" >> /etc/newsyslog.conf
    echo "# _php4_" >> /etc/newsyslog.conf
    echo "/var/log/php/php4_error.log   httpd.httpd  600  10   1024  *     Z /var/run/httpd.pid SIGUSR1" >> /etc/newsyslog.conf
    echo "" >> /etc/newsyslog.conf
    pkill -HUP syslogd
fi
